...
public class LevelPanel : MonoBehaviour {
	...
	void OnEnable()
	{
		...
		{
			this.transform.FindChild("totalCoins").FindChild("Image (2)").GetComponent<RectTransform>().localPosition = new Vector3 (-67, 10, 0);
			
			Debug.Log("LevelPanelEnable()____iPad____");
			
		} else 
		{
			this.transform.FindChild("totalCoins").FindChild("Image (2)").GetComponent<RectTransform>().localPosition = new Vector3 (-90, 10, 0);
